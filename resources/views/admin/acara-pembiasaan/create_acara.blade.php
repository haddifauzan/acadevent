...
@section('title', 'Tambah Acara Sekolah')

@section('content')
<div class="container">
    <div class="card">
        <div class="card-body">
            <h1 class="text-center my-4">Tambah Acara Sekolah Hari : {{ $hari->nama_hari }}</h1>
            <form action="{{ route('acara_sekolah.store') }}" method="POST">
                @csrf
                <!-- Hidden Input untuk id_hari -->
                <input type="hidden" name="id_hari" value="{{ $id_hari }}">
                
                <div class="form-group mb-3">
                    <label for="nama_acara">Nama Acara</label>
                    <input type="text" class="form-control @error('nama_acara') is-invalid @enderror" id="nama_acara" name="nama_acara" required value="{{ old('nama_acara') }}">
                    @error('nama_acara')
                ...
                <div class="form-group mb-3">
                    ...
                    <select class="form-select @error('jenis_acara') is-invalid @enderror" id="jenis_acara" name="jenis_acara" required>
                        ...
                        <option value="Pembiasaan" {{ old('jenis_acara') == 'Pembiasaan' ? 'selected' : '' }}>Pembiasaan</option>
                        <option value="Apel" {{ old('jenis_acara') == 'Apel' ? 'selected' : '' }}>Apel</option>
                        <option value="Upacara" {{ old('jenis_acara') == 'Upacara' ? 'selected' : '' }}>Upacara</option>
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="time" class="form-control @error('waktu_mulai') is-invalid @enderror" id="waktu_mulai" name="waktu_mulai" required value="{{ old('waktu_mulai') }}">
                    @error('waktu_mulai')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group mb-3">
                    ...
                    <input type="time" class="form-control @error('waktu_selesai') is-invalid @enderror" id="waktu_selesai" name="waktu_selesai" required value="{{ old('waktu_selesai') }}">
                    @error('waktu_selesai')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
